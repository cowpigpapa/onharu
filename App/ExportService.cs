...
namespace FamilyPlanner
{
    static class ExportService
    {
        ...
        {
            var lines = new List<string> { "날짜,제목,카테고리,하루종일,시작,종료,할일,완료,중요,D-Day 표시,기념일 기준일,반복,알림(분 전),메모,출처" };
            foreach (var item in items.OrderBy(x => x.Start).ThenBy(x => x.Title))
                lines.Add(string.Join(",", new[] {
                    ...
                    Q(item.ShowDday ? "예" : "아니오"), Q(item.Important && item.ShowDday && item.AnniversaryDate.Year >= 1900 ? item.AnniversaryDate.ToString("yyyy-MM-dd") : ""),
                    Q(Recurrence(item)), Q(item.ReminderConfigured && item.ReminderMinutes >= 0 ? item.ReminderMinutes.ToString() : ""),
                    Q(item.Notes), Q(Source(item)) }));
