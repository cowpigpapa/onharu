...
foreach ($token in @('#define AppVersion "2.2.0"', 'ONHARU-2.2.0-Setup', 'Release\ONHARU-2.2.0', 'C43E8BF2-2B16-4CC7-A85B-D18C2AA7D706')) {
    if (-not $installer.Contains($token)) { throw "Installer 2.2 token is missing: $token" }
}
foreach ($token in @("'ONHARU-2.2.0'", 'ONHARU-2.2.0-Setup.exe', 'SHA256SUMS.txt')) {
    if (-not $release.Contains($token)) { throw "Release build token is missing: $token" }
}
if (-not $readme.Contains('ver. 2.2.0')) { throw 'README product version is stale.' }
