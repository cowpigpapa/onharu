...
$config = Join-Path $PSScriptRoot 'app.config'
...
$arguments = @(
  '/nologo', '/target:winexe', '/optimize+', "/out:$output", "/win32icon:$icon", "/win32manifest:$manifest",
...
Copy-Item -LiteralPath $config -Destination ($output + '.config') -Force
