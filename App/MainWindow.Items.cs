...
namespace FamilyPlanner
{
    public partial class MainWindow
    {
        ...
        async void OpenEdit(PlannerItem item)
        {
            ...
            {
                ShowItemNotice(item, GoogleTasks.IsTask(item) ? "Google Tasks에서는 완료 상태만 변경할 수 있습니다." :
            ...
            if (window.DeleteRequested)
            {
                ...
                if (isGoogle && GoogleCalendar.IsConnected)
                {
                    try
                    {
                        if (GoogleTasks.IsTask(item)) await GoogleTasks.DeleteAsync(item);
                        else if (deleteScope == "future") await GoogleCalendar.TrimSeriesBeforeAsync(item);
            ...
            else
            {
                ...
                if (movedCalendar && GoogleCalendar.IsConnected)
                    ...
                    {
                        if (GoogleTasks.IsTask(item)) await GoogleTasks.DeleteAsync(item);
                        else await GoogleCalendar.DeleteAsync(item, window.ApplyToSeries);
                    }
                    catch (Exception ex) { ErrorLog.Write("Delete Google recurrence", ex); ShowItemNotice(item, "Google 캘린더를 변경하지 못했습니다."); return; }
