...
namespace FamilyPlanner
{
    ...
    public static class RecurrenceService
    {
        ...
        public static DateTime NextOccurrence(PlannerItem item, DateTime current)
        {
            ...
            if (item.RecurrenceFrequency == "yearly" && item.RecurrenceMode == "yearly_nth" && !string.IsNullOrWhiteSpace(item.RecurrenceDays))
            {
                var year = current.Year + 1;
                var day = NthWeekdayOfMonth(year, item.Start.Month, item.RecurrenceDays);
                return new DateTime(year, item.Start.Month, day).Add(time);
            }
        ...
        public static string GoogleRecurrenceRule(PlannerItem item)
        {
            ...
            else if (item.RecurrenceFrequency == "yearly" && item.RecurrenceMode == "yearly_nth" && !string.IsNullOrWhiteSpace(item.RecurrenceDays)) rule += ";BYMONTH=" + item.Start.Month + ";BYDAY=" + item.RecurrenceDays;
            else if (item.RecurrenceFrequency == "yearly") rule += ";BYMONTH=" + item.Start.Month + ";BYMONTHDAY=" + item.Start.Day;
