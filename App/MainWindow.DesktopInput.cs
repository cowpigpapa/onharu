...
namespace FamilyPlanner
{
    public partial class MainWindow
    {
        void HandleDesktopAction(int action, int value)
        {
            ...
            if (action == 105) { Store.SaveSettings(settings); return; }
            ...
            else if (action == 26) { settings.Opacity = Math.Max(.45, Math.Min(.98, value / 100.0)); Opacity = settings.Opacity; Store.SaveSettings(settings); }
            else return;
            RenderAll();
        }
        ...
        void HitTestDesktop(int packedPoint, bool doubleClick)
        {
            ...
            while (current != null)
            {
                ...
                if (element != null && (element.Tag as string == "open_pending_sync" || element.Tag as string == "open_anniversary")) { target = element.Tag; break; }
                if (element != null && (element.Tag is DateTime || element.Tag is PlannerItem || element.Tag is ItemHitTarget)) { target = element.Tag; break; }
        ...
        void AdjustDesktopOpacity(int packedPoint, bool save, bool requireHit)
        {
            ...
            settings.Opacity = slider.Value; Opacity = settings.Opacity;
        ...
        static DependencyObject FindDesktopElement(Visual root, DependencyObject parent, Point point)
        {
            for (var index = VisualTreeHelper.GetChildrenCount(parent) - 1; index >= 0; index--)
            {
                ...
                if (element.Tag is DateTime || element.Tag is PlannerItem || element.Tag is ItemHitTarget) return child;
            }
            return null;
        }
        ...
        void SelectDesktopDate(int index)
        {
            ...
            var offset = mondayFirst ? ((int)shownMonth.DayOfWeek + 6) % 7 : (int)shownMonth.DayOfWeek;
