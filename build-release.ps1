...
$stage = Join-Path $releaseRoot 'ONHARU-2.2.0'
...
Get-FileHash -LiteralPath (Join-Path $installerOutput 'ONHARU-2.2.0-Setup.exe') -Algorithm SHA256 |
    ForEach-Object { $_.Hash + '  ONHARU-2.2.0-Setup.exe' } | Set-Content -LiteralPath (Join-Path $installerOutput 'SHA256SUMS.txt') -Encoding ascii
...
Write-Host "Installer: $(Join-Path $installerOutput 'ONHARU-2.2.0-Setup.exe')"
