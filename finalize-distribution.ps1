...
$installer = Join-Path $installerDir 'ONHARU-2.2.0-Setup.exe'
$hashFile = Join-Path $installerDir 'SHA256SUMS.txt'
$zip = Join-Path $installerDir 'ONHARU-2.2.0-Installer.zip'
...
$zipHash + '  ONHARU-2.2.0-Installer.zip' | Set-Content -LiteralPath (Join-Path $installerDir 'ONHARU-2.2.0-Installer.zip.sha256.txt') -Encoding ascii
