...
인증서를 발급받기 전에는 실제 공개 신뢰 서명을 만들 수 없다. 인증서가 준비되면 ONHARU.exe, Onharu.LayerHost.exe, Onharu.DesktopHook.dll을 먼저 서명하고, 이 파일을 포함해 설치판을 다시 만든 다음 설치판 자체를 마지막으로 서명하고 RFC 3161 타임스탬프를 검증한다.
