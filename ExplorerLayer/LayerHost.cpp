...
static HWND FindIconList(HWND progman, HWND* defViewOut = nullptr)
{
    HWND defView = progman ? FindWindowExW(progman, nullptr, L"SHELLDLL_DefView", nullptr) : nullptr;
    if (defViewOut) *defViewOut = defView;
...
static BOOL WINAPI ConsoleHandler(DWORD type)
{
    if (type == CTRL_C_EVENT || type == CTRL_CLOSE_EVENT) {
        ...
        SendStop(FindWindowW(L"Progman", nullptr));
        return TRUE;
    }
    return FALSE;
}
...
int wmain(int argc, wchar_t** argv)
{
    ...
    for (;;)
    {
        ...
        DWORD processId = 0;
        DWORD threadId = GetWindowThreadProcessId(progman, &processId);
        ...
        DWORD_PTR ignored = 0;
        SendMessageTimeoutW(progman, RegisterWindowMessageW(ONHARU_INIT_MESSAGE), 0, 0,
            SMTO_ABORTIFHUNG, 3000, &ignored);
        HWND defView = nullptr;
        HWND iconList = FindIconList(progman, &defView);
        ...
        while (attached && IsWindow(progman) && IsWindow(iconList) && GetPropW(iconList, ONHARU_LISTVIEW_PROPERTY) &&
