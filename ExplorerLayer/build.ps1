...
$command = 'call "' + $vcvars + '" && cd /d "' + $here + '" && cl' + $common + '/LD DesktopHook.cpp user32.lib gdi32.lib msimg32.lib comctl32.lib /link ' + $libraries + ' /OUT:OnharuV3.DesktopHook.dll && cl' + $common + 'LayerHost.cpp user32.lib /link ' + $libraries + ' /SUBSYSTEM:WINDOWS /ENTRY:wmainCRTStartup /OUT:OnharuV3.LayerHost.exe'
cmd.exe /d /c $command
