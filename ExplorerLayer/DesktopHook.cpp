...
static bool DrawSharedFrame(HDC dc)
{
    ...
    const BYTE opacity = static_cast<BYTE>(header->reserved[0] > 255 ? 255 : header->reserved[0] == 0 ? 255 : header->reserved[0]);
    if (!EnsureCompositeSurfaces(dc, header->width, header->height)) return false;
    if (!g_baseValid || g_surfaceLeft != panel.left || g_surfaceTop != panel.top) {
        if (!BitBlt(g_baseDC, 0, 0, header->width, header->height, dc, panel.left, panel.top, SRCCOPY)) return false;
        g_surfaceLeft = panel.left; g_surfaceTop = panel.top; g_baseValid = true; g_finalGeneration = 0;
    }
    if (g_finalGeneration != generation || g_finalOpacity != opacity) {
        BitBlt(g_finalDC, 0, 0, header->width, header->height, g_baseDC, 0, 0, SRCCOPY);
        AlphaBlend(g_finalDC, 0, 0, header->width, header->height,
            g_cacheDC, 0, 0, header->width, header->height,
            BLENDFUNCTION { AC_SRC_OVER, 0, opacity, AC_SRC_ALPHA });
    ...
    UINT_PTR, DWORD_PTR)
{
    ...
    if ((message == WM_LBUTTONDOWN || message == WM_LBUTTONDBLCLK) && g_frameView && g_desktopActionMessage) {
        ...
        if (icon < 0 && x >= 0 && y >= 0 && x < header->width && y < header->height) {
            ...
            const int hitKind = HitKindAt(header, x, y);
            if (message == WM_LBUTTONDOWN && (hitKind == 2 || hitKind == 3)) {
